...
# compute peak metrics during challenge phase (stroop only) ---------------

data_physio_peak_times_strooponly <- left_join(
...
# join data from all phases (peak version, stroop only) -------------------

# compute peak metrics with challenge phase reflecting stroop only
data_physio_peak_strooponly <- as.data.frame(
  ...
  rbind(
    ...
    # stress task physio data - peak values for specific metrics only
    data_physio_peak_times_strooponly %>%
      mutate(phase = 'peak') %>%
      select(label_subject, phase,
             HR_mean = HR_mean_peak, 
             resp_rate = resp_rate_peak, 
...
# compute reactivity (peak version, stroop only) --------------------------

# (reactivity = difference between baseline and challenge phases)

data_physio_reactivity_strooponly <- data_physio_peak_strooponly %>%
  filter(phase %in% c('baseline', 'peak')) %>%
  pivot_wider(names_from = phase,
              names_glue = "{.value}_{phase}",
              values_from = c(HR_mean, 
...
# bind demographic data to physio data (peak version, stroop only) --------

data_physio_peak_strooponly <- data_physio_peak_strooponly %>%
  rowwise() %>%
  mutate(phase = str_to_title(phase)) %>%
  left_join(data_LCA %>% select(label_subject, age_group, gender),
            by = 'label_subject') %>%
...
rm(data_physio_interval, data_physio_peak_times, data_physio_peak_times_strooponly)
...
rm(data_physio_stress_YA, data_physio_stress_OA)


# subset challenge phase data (avg version, stroop only) ------------------

# (use only stroop for younger)
data_physio_stress_YA <- data_physio %>%
  filter(age_group == 'YA') %>%
  filter(phase == 'stroop') %>%
  group_by(label_subject, label_session) %>%
...
data_physio_stress_strooponly <- as.data.frame(
  rbind(data_physio_stress_YA,
        data_physio_stress_OA)
)
...
# join data from all phases (avg version, stroop only) --------------------

# (not computing reactivity from these data, they are for visualization
# and testing effectiveness of acute stress induction protocol, see script 6_)
data_physio_avg_strooponly <- as.data.frame(
...
data_physio_reactivity_avg_strooponly <- data_physio_avg_strooponly %>%
  filter(phase %in% c('Baseline', 'Challenge')) %>%
  pivot_wider(names_from = phase,
              names_glue = "{.value}_{phase}",
              values_from = c(HR_mean,
