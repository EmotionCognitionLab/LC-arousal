...
# load PLS (peak) results -------------------------------------------------

# read in PLS output generated in matlab
pls_out_all_peak <- readMat(here('data', 'derivatives', 'PLS_all_output_peak.mat'))


# plot PLS results for all phases / caudal LC / LV #1 ---------------------

n_agegr <- pls_out_all_peak$pls.result.all.caudal[,,1]$num.subj.lst
nsubs_YA <- n_agegr[1]
nsubs_OA <- n_agegr[2]
rm(n_agegr)
...
salience_all_caudal <- data.frame(
  usc = pls_out_all_peak$pls.result.all.caudal[,,1]$usc[,1],
  behavdata = pls_out_all_peak$pls.result.all.caudal[,,1]$stacked.behavdata[,1],
...
data_LV_all_caudal <- data.frame(
  ...
  boot_ratio = pls_out_all_peak$pls.result.all.caudal[,,1]$boot.result[,,1]$compare.u[,1]
)
...
fig_pls_LV_all_caudal <- ggplot(data = data_LV_all_caudal,
  ...
  labs(x = '', y = 'Bootstrap ratio',
       subtitle = paste('Latent variable #1 (p=', 
                        round(pls_out_all_peak$pls.result.all.caudal[,,1]$perm.result[,,1]$sprob[1], 3), 
                        ')',
                        sep = '')) +
  theme_apa() +
...
fig_pls_caudal_peak <- ggarrange(fig_pls_LV_all_caudal, 
...
ggsave(filename = here('figures', 'figure6_pls_peak.svg'),
       plot = fig_pls_caudal_peak,
       device = 'svg',
       bg = 'white',
       width = 13, height = 4,
       dpi = 600)
